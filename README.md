...
- If a `WEBUI_SECRET_KEY` environment variable is not provided, Open WebUI looks for `.webui_secret_key` in the current working directory or a directory defined using the `WEBUI_SECRET_KEY_FILE` environment variable. If not found, a new key file is created in this location.
...
- Set `WEBUI_SECRET_KEY` to read the contents of `%DATA_DIR%\.webui_secret_key`.
